...
(defvar ar-mark-decorators nil)
(make-variable-buffer-local 'ar-mark-decorators)
...
Optional ENFORCE-REGEXP: search for regexp only."
  (unless (eobp)
    (let* ((orig (point))
	   ...
	   (regexpvalue (if (member regexp (list 'ar-def-re 'ar-def-or-class-re 'ar-class-re))
